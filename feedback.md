...
<table>
  ...
  <tr>
   ...
   <td>
      yes, except for the send(message) method
   </td>
  </tr>
  ...
  <li>Complex logic is broken into smaller helper methods. (There aren’t more than one or two methods that are 30 lines or more)
   ...
   <td>
      yes; the methods are small enough that they don't necessarily need helper methods. There is a helper method in the slack.rb file for the input prompt
   </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      no - only 11 commits. Commits could be more meaningful - try focusing on specific problems you solve with each commit
    </td>
  </tr>
</table>
...
<table>
  ...
  <tr>
   ...
   <td>
      no, list users/channels is printing out the entire API request instead of formatted details, and also sends a message of the headers to a slack channel
   </td>
  ...
  <tr>
   ...
   <td>
      yes; it would be good to add feedback when an invalid user/channel is selected but the program handles this by keeping the previous valid selection
   </td>
  </tr>
</table>
